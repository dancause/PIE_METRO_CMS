...
def demande_recuperation_motpasse():
    data = "La demande de récupération a été envoyée"
...
def nouveau_usager(token):
    ...
    if res != None:
        ...
        return render_template('error_html.html', error_html="401",
                               error_message="Non autorisée"), 401
...
def envoyer_invitation():
    ...
    if get_db().valider_courriel(courriel) is False:
        data = "Il y a déja un compte associé à ce courriel"
...
def invitation():
    ...
    return render_template('login.html',
                           message='Votre code usager a été créer')
...
def search_term():
    ...
    if verifierLangue() == 'FR':
        resp = make_response(render_template('temp_intro_articles.html',articles=articles,title='Resultats de recherche',recherche=rechercher))
    else:
...
def page_not_found400(e):
    return render_template('error_html.html', error_html="400",
                           error_message="Requête erronée"), 400
...
def page_not_found405(e):
    return render_template('error_html.html', error_html="405",
                           error_message="Méthode requête non autorisée"), 405
